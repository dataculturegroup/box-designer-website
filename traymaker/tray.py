...
class Tray():
    '''
    Handles actually drawing of the notched tray to a file.  This class passes everything around
    ...
    def __init__(self,file_path,width,height,depth,thickness,
           cut_width,notch_length,bounding_box=True,tray=True):
        self._logger = logging.getLogger(__name__)
        self._file_path = file_path
        self._desired_size = { 'w': float(width), 'h': float(height), 'd': float(depth) }
        self._logger.debug(" desired tray size: (w=%.2f, h=%.2f, d=%.2f)" % (self._desired_size['w'],self._desired_size['h'],self._desired_size['d']))
    ...
    def _compute_dimensions(self):
        ...
        self._box_pieces_size = { 'w': self._size['d']*2.0 + self._size['w'],
                                  'h': self._size['h']*2.0 + self._size['d'] }
        ...
        if self._tray:
            ...
            self._bounding_box_size = { 'w': self._box_pieces_size['w']+self._margin*2,
                                    'h': self._box_pieces_size['h']+self._margin*2 }
            self._doc_size = {'w': self._box_pieces_size['w']+self._margin*4,
                          'h': self._box_pieces_size['h']+self._margin*4 }
        else:
            self._box_pieces_size = { 'w': self._size['d']*2.0 + self._size['w'],
    ...
    def _initialize_document(self):
        ...
        self._doc.setAuthor(traymaker.APP_NAME+" "+traymaker.APP_VERSION)
        ...
        self._doc.drawString(15*mm,20*mm, "Produced by "+traymaker.APP_NAME+" v"+traymaker.APP_VERSION+
            " on "+time.strftime("%m/%d/%Y")+" at "+time.strftime("%H:%M:%S"))
        self._doc.drawString(15*mm,15*mm, traymaker.WEBSITE_URL )
